...
class JenkinsPlugin(object):
    ...
    def _download_updates(self):
        ...
        if download_updates:
            ...
            updates_file_tmp = tempfile.mkstemp()

            updates_file_fd = updates_file_tmp[0]
            updates_file = updates_file_tmp[1]
            fd = os.fdopen(updates_file_fd, 'wb')
            fd.write(r.read())
    ...
    def _write_file(self, f, data):
        ...
        try:
            fd = os.fdopen(tmp_fd, 'wb')
        except IOError:
            e = get_exception()
            self.module.fail_json(
                msg='Cannot open the temporal plugin file %s.' % tmp_f_name,
                details=str(e))
        ...
        try:
            ...
            e = get_exception()
            self.module.fail_json(
                msg='Cannot close the temporal plugin file %s.' % tmp_f_name,
                details=str(e))

        # Move the file onto the right place
        self.module.atomic_move(tmp_f_name, f)
