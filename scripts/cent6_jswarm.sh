...
ansible -i /dev/null localhost -m docker_container -a "image=greghellings/centos_base:6 name=jswarm detach=true tty=true command=/bin/bash"
echo "Building container with Ansible"
ansible-playbook -i "${CINCH}/inventory/cent6_jswarm_docker" "${CINCH}/cinch/site.yml" \
